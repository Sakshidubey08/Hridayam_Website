...
const Privacy = () => {
  ...
  useEffect(() => {
    ...
    const fetchPrivacyPolicy = async () => {
      try {
        ...
        const sanitizedTerms = DOMPurify.sanitize(response.data.data.privacy_policy || 'No terms and conditions found.', {
          ALLOWED_TAGS: ['p', 'b', 'i', 'strong', 'ul', 'li', 'ol', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6'], // Include heading tags

        });
        
        setPrivacy(sanitizedTerms);
      } catch (err) {
        // Handle errors
  ...
  return (
    <>
      <Header />
      <div>
      <h1 className='top text-center '>Privacy Policy</h1>
        <div className='mt-4' dangerouslySetInnerHTML={{ __html: privacy }}></div>
      </div>
      <Footer />
    </>
  );
